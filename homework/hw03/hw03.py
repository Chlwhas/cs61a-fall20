...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        return tree(total_weight(m), [totals_tree(end_item) for end_item in [end(left(m)), end(right(m))]])
...
def has_path(t, word):
    ...
    if len(word) == 1:
        ...
        return any([has_path(branch, word) for branch in branches(t)])
...
def quadratic(x, a, b, c):
    ...
    if a == 0:
        return interval(min(value_left, value_right), max(value_left, value_right))
    else:
        point = -b / (2 * a)
        value_point = f(point)
        if left <= point <= right:
            return interval(min(value_point, value_left, value_right), max(value_point, value_left, value_right))
        else:
            return interval(min(value_left, value_right), max(value_left, value_right))
