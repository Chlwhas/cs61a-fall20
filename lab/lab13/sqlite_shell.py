...
def sql_commands(read_line):
    ...
    while True:
        ...
        if j < len(line):
            if not in_string:
                j = min(map(lambda k: k if k >= 0 else len(line),
                            [line.find('"', j), line.find("'", j), line.find(';', j)]))
...
class SuppressKeyboardInterrupt(object):
    ...
    def flush(self, *args):
        while True:
            try:
                return self.base.flush(*args)
            except KeyboardInterrupt as ex:
                self.mark_interrupt(ex)

    def write(self, *args):
        while True:
            try:
                return self.base.write(*args)
            except KeyboardInterrupt as ex:
                self.mark_interrupt(ex)
    ...
    def writelines(self, *args):
        while True:
            try:
                return self.base.writelines(*args)
            except KeyboardInterrupt as ex:
                self.mark_interrupt(ex)
...
def parse_escaped_strings(s, encoding='utf-8', pattern=re.compile(
    ...
    for match in pattern.finditer(s):
        ...
        if len(m) > 0 and m[0] in "\'\"'":
            m = escape_pattern.sub(lambda m: (lambda decoded: m.group(1) if m.group(0) == decoded else decoded)(
                m.group(0).encode(encoding).decode('string-escape')), match.group(1))
        yield m
...
""".lstrip());
                    ...
                    query = "SELECT sql || ';' FROM sqlite_master WHERE type = :type" + (
...
def main(program, *args,
         **kwargs):  # **kwargs = dict(stdin=file, stdout=file, stderr=file); useful for callers who import this module
    ...
    (stdin, stdout, stderr) = (
    kwargs.pop('stdin', sys.stdin), kwargs.pop('stdout', sys.stdout), kwargs.pop('stderr', sys.stderr))
    if False and len(args) == 0: return argparser.print_usage(stderr)
    return run(stdin, stdout, SuppressKeyboardInterrupt(stderr) if isatty(stderr) else stderr,
