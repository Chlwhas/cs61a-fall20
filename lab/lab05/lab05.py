...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    latitude_diff = get_lat(city_a) - get_lat(city_b)
    longitude_diff = get_lon(city_a) - get_lon(city_b)
    return sqrt(latitude_diff ** 2 + longitude_diff ** 2)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    distance_a = distance(city_a, city)
    distance_b = distance(city_b, city)

    close_city = city_a if distance_a <= distance_b else city_b
    return get_name(close_city)
...
def add_trees(t1, t2):
    ...
    while i < len(branches(t1)) and i < len(branches(t2)):
        result_branches = result_branches + [add_trees(branches(t1)[i], branches(t2)[i])]
        i = i + 1
